...
def package_skill(root: Path, output: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise
    return {"output": str(output), "file_count": len(files), "bytes": output.stat().st_size, "sha256": hashlib.sha256(output.read_bytes()).hexdigest(), "lint_status": report["status"], "warnings": [i for i in report["issues"] if i["level"] == "warning"], "excluded": excluded, "installed": False, "runtime_validation": "not_run"}
